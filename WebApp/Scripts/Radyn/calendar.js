...
function validateDate(control) {
    ...
    if (control.value.length != 10) {
        message = 'طول رشته تاریخ وارد شده باید 10 کاراکتر باشد.\nلطفا به مثال نمایش داده شده توجه نمایید.';
    }
    var split = control.value.split('/');
    if (split.length != 3)
        message = 'فرمت ورود اطلاعات صحیح نمی باشد.\nلطفا به مثال نمایش داده شده توجه نمایید.';
    if (split[1] < '01' || split[1] > '12')
        message = 'مقدار ماه باید بین 1 تا 12 باشد.';
    if (split[2] < '01' || split[2] > '31')
        message = 'مقدار روز باید بین 1 تا 31 باشد.';
    if (message != '') {
        alert(message);
        control.focus();
    }
